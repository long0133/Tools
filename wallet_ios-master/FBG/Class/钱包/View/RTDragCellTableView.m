...
@property (nonatomic, strong) NSIndexPath *originalDidIndexPath;
    ...
    if ([UserSignData share].user.isLogin) {
        ...
        switch (longPressState) {
            case UIGestureRecognizerStateBegan:{  //手势开始，对被选中cell截图，隐藏原cell
                ...
                _originalDidIndexPath = _originalIndexPath;
                NSLog(@"_originalDidIndexPath = %@", _originalDidIndexPath);
            ...
            default: {                             //长按手势结束或被取消，移除截图，显示cell
                if (_relocatedIndexPath && ![_relocatedIndexPath isEqual:_originalDidIndexPath]) {
...
- (void)updateDataSource{
    ...
    if ([self.delegate respondsToSelector:@selector(tableView:moveObjectFrom:to:)]) {
        [self.delegate tableView:self moveObjectFrom:_originalIndexPath.row to:_relocatedIndexPath.row];
    }
}
