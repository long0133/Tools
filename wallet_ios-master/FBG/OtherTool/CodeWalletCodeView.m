...
@implementation CodeWalletCodeView
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self)
    {
        self = [CodeWalletCodeView loadViewFromXIB];
        self.frame = frame;
...
- (IBAction)SureButtonCilick:(id)sender
{
    //确定按钮
    [self  canel];
    if ([self.delegate respondsToSelector:@selector(CodeWalletCodeViewsureButtonCilick)])
    {
        [self.delegate CodeWalletCodeViewsureButtonCilick];
    }
