...
- (void)setIsHaveData:(BOOL)isHaveData {
    ...
    self.boxView.backgroundColor = _isHaveData ? [UIColor lightGrayColor] : [UIColor whiteColor];
}
...
- (UIView *)boxView {
    if (!_boxView) {
        _boxView = [[UIView alloc] initWithFrame:CGRectMake((CGRectGetWidth(self.frame) - 5) / 2.0, CGRectGetMaxY(self.textLabel.frame), 5, 5)];
