...
- (void)setMonthArray:(NSArray *)monthArray {
    ...
    for (DAYComponentView *componentView in self.componentViews) {
        componentView.isHaveData = NO;
        for (NSString *day in _monthArray) {
            if (day.integerValue == componentView.textLabel.text.integerValue) {
                componentView.isHaveData = YES;
                break;
            }
        }
    }
