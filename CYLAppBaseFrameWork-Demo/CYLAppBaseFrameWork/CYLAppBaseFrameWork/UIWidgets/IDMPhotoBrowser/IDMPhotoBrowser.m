...
- (id)init {
    if ((self = [super init])) {
        ...
        _pageControl.center = CGPointMake(kScreenWidth/2, kScreenHeight - 40);
        [self.view addSubview:_pageControl];
        
        _dismissOnTouch = true;
        _displayToolbar = false;
        _displayDoneButton = false;
    }
...
- (id)initWithPhotoURLs:(NSArray *)photoURLsArray {
    if ((self = [self init])) {
		...
		_photos = [[NSMutableArray alloc] initWithArray:photosArray];
        _pageControl.numberOfPages = photoURLsArray.count;
	}
	return self;
}
